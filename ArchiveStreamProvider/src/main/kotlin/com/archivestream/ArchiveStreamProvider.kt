...
/**
 * Archive.org kamu mali (public domain) icerik provider'i.
 * Tum aramalar ve ana sayfa yalnizca kamu mali koleksiyonlarla sinirlidir.
 */
class ArchiveStreamProvider : MainAPI() {
    override var mainUrl = "https://archive.org"
    override var name = "ArchiveStream (Kamu Mali)"
    override val supportedTypes = setOf(TvType.Movie)
    override var lang = "en"
    override val hasMainPage = true

    private val publicDomainCollections = listOf(
        "feature_films",     // Kamu mali filmler
        "Film_Noir",         // Film noir
        "SciFi_Horror",      // Sci-fi / korku
        "classic_cartoons",  // Klasik cizgi filmler
        "prelinger",         // Prelinger arsivi (belgesel/ephemeral)
        "nasa"               // NASA video arsivi
    )

    private val collectionFilter by lazy {
        publicDomainCollections.joinToString(" OR ") { "collection:($it)" }
    }

    private val mapper by lazy {
        // Metadata farkli formatlarda gelebilir, burada ele alinir.
    ...
    override val mainPage = mainPageOf(
        ...
        "SciFi_Horror" to "Sci-Fi / Korku",
        "classic_cartoons" to "Klasik Cizgi Filmler",
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        return try {
            val q = "collection:(${request.data})+AND+mediatype:(movies)"
            val responseText = app.get(
                "$mainUrl/advancedsearch.php?q=$q&fl[]=identifier&fl[]=title&fl[]=mediatype&rows=26&page=$page&output=json&sort[]=downloads+desc"
            ).text
    ...
    override suspend fun search(query: String, page: Int): SearchResponseList? {
        return try {
            val q = "${query.encodeUri()}+AND+($collectionFilter)+AND+mediatype:(movies)"
            val responseText = app.get(
                "$mainUrl/advancedsearch.php?q=$q&fl[]=identifier&fl[]=title&fl[]=mediatype&rows=26&page=$page&output=json"
            ).text
    ...
    private data class SearchEntry(
        ...
        fun toSearchResponse(provider: ArchiveStreamProvider): SearchResponse {
    ...
    private data class MetadataResult(
        ...
        suspend fun toLoadResponse(provider: ArchiveStreamProvider): LoadResponse {
            ...
            return if (videoFiles.distinctBy { getUniqueName(it.name) }.count() <= 1 || type == TvType.Audio) {
                ...
                // Cok videolu item'lar oynatma listesi olarak gosterilir (izleme konumu icin).
